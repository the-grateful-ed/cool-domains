...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) ERC721 (".bit Name Service", "BNS") payable {
    ...
    function register(string calldata name) public payable {
        ...
        if (valid(name) == false) revert InvalidName(name);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the bit name service", "image": "data:image/svg+xml;base64,',
